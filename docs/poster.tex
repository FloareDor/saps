...
\begin{columns}[t]
  \begin{column}{0.82\linewidth}
    ...
    {\Large Sai Ravi Teja Gangavarapu \quad Riya Elizabeth John \quad Vedanti Kshirsagar}\\[0.2em]
...
\begin{column}{0.30\linewidth}
  ...
  \begin{block}{Results (LLaDA-8B, GSM8K)}
    ...
    {\small Pruning degrades HumanEval severely (vanilla 36.6\%\,$\to$\,Sparse 12.2\%\,$\to$\,SAPS 9.8\%;
    $\Delta{\approx}25$\,pp, $p{\ll}0.001$) and MBPP modestly ($\Delta{\approx}5$\,pp, $p{\approx}0.09$).
    SAPS and Sparse are tied on both (HumanEval: $p{=}0.48$; MBPP: $p{=}0.78$).
    Step-aware scheduling helps \emph{math}, not code, at $r_{\min}{=}0.1$.}
  ...
  \begin{block}{Takeaway}
    ...
    \vspace{0.4em}
    \textit{Ablations:} layer-aware $r(t,\ell)$ — linear\_up: $-$1.6\,pp math / $-$1.8\,pp code;
    entropy: $-$5.8\,pp math / $-$7.3\,pp code (2.4\% HumanEval).
    \textbf{Temporal schedule dominates spatial allocation.}\\[0.2em]
    softer $r_{\min}{=}0.2$: code flat (HumanEval 9.1\%, MBPP 29.0\%) — degradation is structural.\\[0.2em]
    \textit{Future work:} learned schedules; longer-context evaluation.
  \end{block}
